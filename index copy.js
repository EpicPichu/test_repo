...
async function bedwars(username, interval = 'Lifetime', gamemode = 'All_Modes') {
    ...
    try {
        ...
        if (profile.clan) {
            guild = true;
            gName = profile.clan.name;
            gTag = profile.clan.tag;
            gLevel = String(profile.clan.leveling.level);
            gMembers = String(profile.clan.members.length);
        } else {
            guild = false;
        }
